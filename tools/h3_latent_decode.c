...
int main(int argc, char **argv) {
    if (argc != 5 && argc != 6)
        die("MODEL_ROOT SHADER INPUT_LATENT OUTPUT_MP4 [SOURCE_AUDIO_MP4]");
    char weights[4096];
    ...
    int encoded = argc == 6 ?
        h3_ffmpeg_write_rgb24_with_source_audio(
            argv[4], rgb, frames.frames, frames.width, frames.height, 24,
            argv[5], H3_VIDEO_PRESET_SLOW, 18, error, sizeof(error)) :
        h3_ffmpeg_write_rgb24(
            argv[4], rgb, frames.frames, frames.width, frames.height, 24,
            H3_VIDEO_CODEC_H264, H3_VIDEO_PRESET_SLOW, 18,
            error, sizeof(error));
    ...
    printf("decode latent_shape=1x24x%dx%dx%d frames=%d pixels=%dx%d "
           ...
           (unsigned long long)frames.gpu_stats.host_tensor_reads,
           (unsigned long long)frames.gpu_stats.host_tensor_writes,
